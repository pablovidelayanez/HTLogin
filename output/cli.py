    ...
    def print_summary(result: ScanResult) -> None:
        ...
        if hasattr(result, 'baseline_login') and result.baseline_login:
            ...
            if baseline.get("success"):
                print(f"{Colors.GREEN}✓{Colors.RESET} {Colors.BOLD}Test Account Login{Colors.RESET}: {Colors.GREEN}Successful{Colors.RESET}")
                print(f"  {Colors.DIM}Login successful, headers captured{Colors.RESET}")
                print(f"  {Colors.DIM}Confidence:{Colors.RESET} {baseline.get('confidence_level', 'N/A')} ({baseline.get('confidence_score', 0)})")
                if baseline.get("session_cookie"):
                    print(f"  {Colors.DIM}Session Cookie:{Colors.RESET} {baseline.get('session_cookie')}")
                if baseline.get("redirect_url"):
