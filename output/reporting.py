...
class ReportGenerator:
    ...
    def _generate_target_html(self, result: Dict[str, Any]) -> str:
        if "error" in result:
            error_msg = html.escape(str(result.get('error', '')))
            url = html.escape(str(result.get('url', 'Unknown URL')))
            return f"""
...
"""
        ...
        url_escaped = html.escape(str(result.get('url', 'Unknown')))
        html = f"""
...
"""
        ...
        if 'confidence_level' in test_result:
            ...
            html += f'<span class="confidence {conf_class}">Confidence: {conf_level.title()} ({conf_score})</span>'
            
            if test_result.get('manual_verification_recommended'):
                html += '<span class="manual-verify">⚠ Manual Verification Recommended</span>'
...
"""
        ...
        if 'payload' in test_result and test_result['payload']:
            payload_escaped = html.escape(str(test_result["payload"]))
            html += f'<div><strong>Payload:</strong> <code>{payload_escaped}</code></div>'
        if 'credential' in test_result and test_result['credential']:
            credential_escaped = html.escape(str(test_result["credential"]))
            html += f'<div><strong>Credential:</strong> <code>{credential_escaped}</code></div>'
        if 'total_duration' in test_result:
            html += f'<div><strong>Test Duration:</strong> {test_result["total_duration"]:.2f} seconds</div>'
        
        indicators = test_result.get('details', {}).get('indicators', [])
        if indicators:
            indicators_escaped = [html.escape(str(ind)) for ind in indicators]
            html += f'<div><strong>Indicators:</strong> {", ".join(indicators_escaped)}</div>'
        
        if test_result.get('limitations'):
            limitations_escaped = html.escape(str(test_result["limitations"]))
            html += f'<div class="limitations"><strong>Limitations:</strong> {limitations_escaped}</div>'
